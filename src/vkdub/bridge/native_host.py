"""VK Dub Studio — Native Messaging Host Bridge Logic.
        ...
        while self.running:
            if not self.connected:
                try:
                    ...
                    logger.debug("Local Agent not available yet (%s). Retrying in 2s...", exc)
                    time.sleep(2.0)
            else:
                time.sleep(1.0)

    def send_loop(self) -> None:
        """Read from send_queue and transmit newline-delimited JSON to Local Agent."""
        while self.running:
            try:
            ...
            if not self.connected or not self.sock:
                logger.warning("Dropped message, Local Agent not connected: %s", msg)
                continue
            ...
            try:
                ...
                logger.error("Failed to send message to Local Agent: %s", exc)
                self._disconnect_socket()

    def send_to_agent(self, msg: dict) -> None:
        """Enqueue message for delivery to Local Agent."""
        try:
            self.send_queue.put_nowait(msg)
        except queue.Full:
            logger.warning("Send queue full, dropping message: %s", msg)

    def _recv_loop(self) -> None:
        """Receive messages from Local Agent and forward to standard output."""
        buf = ""
        try:
            while self.running and self.connected and self.sock:
                chunk = self.sock.recv(4096)
                if not chunk:
                    logger.info("Local Agent closed socket connection.")
                    break
